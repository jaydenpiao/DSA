...
bool BinaryTree<T>::isOrdered() const
{
  T prev;
  bool isFirst = true;
  return isOrdered(root, prev, isFirst);
}

template <typename T>
bool BinaryTree<T>::isOrdered(const Node* subRoot, T& prev, bool& isFirst) const {
  if (!subRoot) return true;

  if (!isOrdered(subRoot->left, prev, isFirst)) return false;

  if (!isFirst && subRoot->elem < prev) return false;

  prev = subRoot->elem;
  isFirst=false;
  return isOrdered(subRoot->right, prev, isFirst);
