...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    transform(word1copy.begin(), word1copy.end(), word1copy.begin(), ::toupper);
    transform(word2copy.begin(), word2copy.end(), word2copy.begin(), ::toupper);

    if (dict.find(word1copy) == dict.end() || dict.find(word2copy) == dict.end()) return false;
    return dict.at(word1copy) == dict.at(word2copy);
}
